...
def next_element(bst, n):
    ...
    """
    def helper(bst, list, n):
        list += [bst.label]
        if bst.left is BST.empty and bst.right is BST.empty:
            new_list = []
            for i in list:
                if i > n:
                    new_list.extend([i])
            return min(new_list)
        elif bst.label > n and bst.left is not BST.empty:
            return helper(bst.left,list, n)
        else:
            return helper(bst.right, list, n)
    return helper(bst, [], n)
