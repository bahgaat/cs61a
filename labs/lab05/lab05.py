...
def filter(pred, seq):
    ...
    [2, 4]
    """
    return [sq for sq in seq if pred(sq) == True]
...
def acorn_finder(t):
    ...
    if label(t) == 'acorn':
        ...
        list_of_Branches = [acorn_finder(branch)for branch in branches(t)]
    if True in list_of_Branches:
        return True
    else:
        return False
