...
def flatten(lst):
    ...
    for i in join_lst:
        if type(i) == list:
            join_join_list = join_join_list + i
        else:
            join_join_list.append(i)
    return join_join_list
...
def merge(lst1, lst2):
    ...
    join_lst = lst1 + lst2
    merge_join_lst = []
    num_larger_than_i_to_replace_it = 10
    for i in join_lst:
        i = i
        for x in join_lst:
            if i > x:
                ...
                i = i
        merge_join_lst.append(i)
        join_lst.remove(i)
        join_lst.append(num_larger_than_i_to_replace_it)
    return merge_join_lst
######################
### Connect N Game ###
######################
...
def put_piece(board, max_rows, column, player):
    ...
    while rows >= 0:
        if get_piece(board, rows , column) == '-':
            adding_player_to_column_in_row = replace_elem(board[rows], column, player)
            new_board = replace_elem(board, rows, adding_player_to_column_in_row)
...
def print_board(board, max_rows, max_cols):
    ...
    while x >= 0:
        if y < 0:
            ...
            get_column_from_the_board = get_piece(board, rows - x , column - y )
            y = y - 1
            n = get_column_from_the_board
            r = r + " " + n
            k = r.strip()
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    n = 2
    for x in range(row):
        get_one_column_from_the_board = get_piece(board, row, column - y )
        y = y - 1
        get_another_column_from_the_board = get_piece(board, row, column - y )
        if get_one_column_from_the_board == get_another_column_from_the_board :
            if get_one_column_from_the_board == player and get_another_column_from_the_board == player:
                num_connect = num_connect - n
                n = n - 1
        if num_connect == 0:
            return True
    return False
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    n = 2
    for x in range(row):
        get_one_column_from_the_board = get_piece(board, row , col )
        get_another_column_from_the_board = get_piece(board, row - 1 , col)
        if get_one_column_from_the_board == get_another_column_from_the_board :
            if get_one_column_from_the_board == player and get_another_column_from_the_board == player:
                num_connect = num_connect - n
                n = n - 1
